...
const OrderPage = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                const transactionsResponse = await fetch(
                    "https://thtstore-websitebandogiadung-backend.onrender.com/api/users/mytransactions/customer@example.com", // Thay bằng email thực tế
                    {
                        headers: {
                ...
                setTransactions(transactionsData.success ? transactionsData.data : []);

                if (ordersResponse && ordersResponse.length > 0) {
                    setSelectedOrder(ordersResponse[0]);
                }
            } catch (error) {
    ...
    // Kết hợp orders và transactions
    const combinedList = [
        ...orders.map((order) => ({
            type: "order",
            ...order,
        ...
        ...transactions
            .filter((tx) => tx.status === "SUCCESS" && !orders.some((order) => order.transactionId === tx.transactionId))
            .map((tx) => ({
                type: "transaction",
                _id: tx._id,
                id: tx.transactionId,
                ...
                user_phone: "0326829327", // Lấy từ webhook
                ...
                email: tx.metadata?.customerEmail || "default@example.com",
                items: (tx.metadata?.items || []).map((item) => ({
    ...
    const filteredOrders = combinedList.filter((item) => {
        const phone = item.user_phone || "";
        const orderDate = new Date(item.created_at).toLocaleDateString("vi-VN");

        const matchesPhone = phone.includes(searchText);
        const matchesDate = searchDate ? orderDate.includes(searchDate) : true;
                    ...
                    <div className="bg-white rounded-lg shadow-md">
                        ...
                        <div className="overflow-x-auto">
                            {loading ? (
                              ...
                              ${selectedOrder?.id === item.id ? "bg-blue-50" : ""}
                              ${item.status?.toLowerCase() === "pending" &&
                                                    (new Date() - new Date(item.created_at)) / (1000 * 60 * 60 * 24) > 7
                            ...
                            `}
                                                >
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="text-sm text-gray-900">{item.user_fullName}</div>
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="text-sm font-medium text-gray-900">
                                                            {item.user_phone || "Chưa có số điện thoại"}
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="text-sm font-medium text-gray-900">
                                                            {item.updated_at ? formatDate(item.updated_at) : "Chưa nhận hàng"}
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap">
                              <span
                                ...
                                {getStatusText(item.status)}
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="text-sm text-gray-900">
                                                            {item.type === "order" ? "Đơn hàng" : "Giao dịch"}
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))
                                            ...
                                            <tr>
                                                <td colSpan={10} className="px-6 py-4 text-center text-sm text-gray-500">
                                                    Không tìm thấy đơn hàng hoặc giao dịch nào
                                                </td>
                                            </tr>
                                        )}
                                        </tbody>
                        ...
                        <div className="px-6 py-4 border-t">
                            <div className="text-sm text-gray-500">
                                Hiển thị {filteredOrders.length} trong tổng số {combinedList.length} đơn hàng/giao dịch
                            </div>
                        </div>
                    </div>
                </div>
