...
function ProductAdminPage() {
  ...
  const updateFiltersAndURL = useCallback(
      (newFilters) => {
        ...
        setPriceRange([newFilters.minPrice, newFilters.maxPrice]); // Thêm để cập nhật priceRange
        ...
        if (typeof refetch === "function") { // Kiểm tra typeof là thừa
          refetch(newFilters);
        }
      },
      [navigate, refetch, setFilters, setPriceRange], // Đã sửa danh sách phụ thuộc
  );
        ...
        const handleBuyNow = async (product) => {
          ...
          try {
            console.log("Adding product to cart, product ID:", product._id)
            await addToCart(product, 1)
            await new Promise(resolve => setTimeout(resolve, 500))
            console.log("Cart after add:", cart.map(item => ({ _id: item._id, productId: item.product?._id })))
