...
function ProductsPage() {
  ...
  const handleBuyNow = async (product) => {
    ...
    try {
      ...
      // Chờ một chút để đảm bảo cart được cập nhật
      await new Promise(resolve => setTimeout(resolve, 100))

      console.log("Cart after add:", cart.map(item => ({ id: item._id || item.id, product: item.product?.id })))
      const isProductInCart = cart.some(item => (item._id || item.id || item.product?.id) === product.id)
      if (!isProductInCart) {
